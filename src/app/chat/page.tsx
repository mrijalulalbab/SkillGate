...
export default function ChatPage() {
  ...
  async function loadContacts() {
    ...
    try {
      ...
      // 1. Get UMKMs from the student's applications
      ...
      (appsData || []).forEach((app: any) => {
        const umkm = app.gig?.umkm;
        if (umkm && umkm.id !== user.id) {
          contactMap.set(umkm.id, {
            id: umkm.id,
            name: umkm.full_name,
            businessName: umkm.umkm_profiles?.[0]?.business_name || "Mitra UMKM"
          });
        }
      });
        ...
        if (!contactMap.has(otherId)) {
          ...
          if (uData) {
            contactMap.set(otherId, {
              ...
              businessName: uData.umkm_profiles?.[0]?.business_name || "Pengguna"
            });
          }
        }
      }
  ...
  async function loadMessages(contactId: string) {
    ...
    try {
      ...
      // Query all messages involving current user, then filter client-side to be 100% robust
          ...
          <div className="flex-1 overflow-y-auto">
            {loadingContacts ? (
              ...
              contacts.map((contact) => (
                ...
                >
                  <div className="relative w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center shrink-0 border border-primary/20">
                    <span className="font-bold text-primary">{(contact.name || "?")[0]}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-baseline mb-1">
                      <h3 className="text-sm font-bold text-foreground truncate">{contact.businessName}</h3>
                    </div>
          ...
          {activeContact ? (
            <>
              ...
              <div className="h-16 px-6 bg-white border-b border-border/40 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center border border-primary/20">
                    <span className="font-bold text-primary">{(activeContact.name || "?")[0]}</span>
                  </div>
                  <div>
                    <h3 className="font-bold text-foreground">{activeContact.businessName}</h3>
                    <p className="text-[10px] text-muted-foreground">{activeContact.name}</p>
                  </div>
                </div>
                ...
                {loadingMsg ? (
                  ...
                  messages.map((msg) => {
                    ...
                    return (
                      ...
                      >
                        <div className="bg-white p-3 rounded-2xl shadow-sm border border-border/40" style={isOwnMessage ? { backgroundColor: 'var(--primary)', color: 'white' } : {}}>
              ...
              <div className="p-4 bg-white border-t border-border/40 shrink-0">
                <div className="flex items-end gap-2 bg-muted/30 p-2 rounded-2xl border border-border/50 focus-within:border-primary/50 transition-colors">
                  <button className="p-2 text-muted-foreground hover:text-primary rounded-full shrink-0 mb-0.5">
                  ...
                  <button 
                    ...
                    className="p-3 bg-primary text-primary-foreground rounded-full hover:shadow-md transition-all shrink-0 mb-0.5"
                  >
            ...
            <div className="flex-grow flex flex-col items-center justify-center text-center p-8">
              ...
              <p className="text-muted-foreground max-w-sm">Pilih salah satu kontak di daftar kiri untuk mulai berdiskusi.</p>
            </div>
          )}
        </div>
      </div>
